...
describe('implosion toBits', () => {
    for (const testSet of [
        ...
        },
        {
            ...
            output: `test[${uarray2bits(
                new Uint8Array(new Array(1000/8)).fill(0xff))}]`
        },
        {
            case: "mixed",
            input: [{
            ...
            output: `xtest[7]test[${uarray2bits(
                new Uint8Array(new Array(1000/8)).fill(0xff))}]`
        }
    ]) {
...
async function ensureOutBitVect(inb: InBitVect): Promise<OutBitVect> {
    return {
        outAsBits: (await getImplosion()).fromBits(inb.inAsBits).Ok().map(i => `${i.name}[${i.tags.join(',')}]`).join(""),
    }
}
...
it('ensureOutBitVect', async () => {
    ...
    a[19] = 1;
    expect(await ensureOutBitVect({ inAsBits: "test[111111111111111111B9uiJ2H3McB6kJoyaRsiWrSQoJYf8w9rgdX4pKhgbzvN2WDF6xnhLDJYcWP5QfeCjYbzAXN5j8C1KUasDtxdkxuULnG9WVi9QKYJDF]" })).toEqual({
...
async function ensureInBitVect(inb: string[]): Promise<InBitVect> {
    return {
        inAsBits: (await getImplosion()).toBits([
            {
                name: "test",
                tags: inb
            }
